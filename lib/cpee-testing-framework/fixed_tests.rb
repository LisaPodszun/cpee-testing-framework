...
module TestHelpers
    ...
    def run_tests_on(settings, testinstance, data, testcase)
        ...
        if testcase != 'custom'
            ...
            testinstance[testcase][:instance_1][:end] = Time.now
            testinstance[testcase][:instance_1][:duration_in_seconds] = testinstance[testcase][:instance_1][:end] - testinstance[testcase][:instance_1][:start]
            puts "Ruby log"
            p ruby_log
            testinstance[testcase][:instance_2][:start] = Time.now
            rust_log = run_test_case(start_url, engine_2, testcase, doc_url_ins_2, data)
            testinstance[testcase][:instance_2][:end] = Time.now
            testinstance[testcase][:instance_2][:duration_in_seconds] = testinstance[testcase][:instance_2][:end] - testinstance[testcase][:instance_2][:start]
            puts "Rust log"
            p rust_log
        else
    ...
    def match_logs(rust_log, ruby_log, missing_events_ruby, missing_events_rust)
        ...
        #rust_log_tags.merge!({1 => 3})
        #ruby_log_tags.merge!({3 => 1})
